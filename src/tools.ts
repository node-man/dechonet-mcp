...
async function postApi(path: string, payload: unknown): Promise<any> {
  const res = await fetch(`${BASE_URL}${path}`, {
    method: 'POST',
...
// Behavior hints per the MCP tool-annotations spec. All DechoNet tools are
// read-only and idempotent; openWorldHint marks the ones that touch the
// network (everything except subnet_calc).
...
const annotate = (title: string, openWorld = true): ToolAnnotations => ({
  title,
  readOnlyHint: true,
...
function structuredFromInterp(data: any, url: string): Record<string, any> {
  ...
  const sc: Record<string, any> = { status: String(interp?.status ?? 'unknown'), reportUrl: url };
  if (interp?.insight?.summary) sc.summary = String(interp.insight.summary);
  if (interp?.kpis?.length) sc.kpis = interp.kpis.map((k: any) => ({ label: String(k.label), value: String(k.value) }));
  if (interp?.issues?.length) sc.issues = interp.issues.map((i: any) => ({ severity: String(i.severity), key: String(i.key) }));
  if (interp?.actionItems?.length) sc.actions = interp.actionItems.map((a: any) => String(a));
  const grade = interp?.securityGrade ?? interp?.sslGrade;
  if (grade) sc.grade = String(grade);
  const score = interp?.securityScore ?? interp?.sslScore;
  if (typeof score === 'number') sc.score = score;
  return sc;
}

function formatResult(data: any, url?: string): { content: Array<{ type: 'text'; text: string }>; structuredContent?: Record<string, any> } {
  const interp = data.interpretation;
  const lines: string[] = [];
  ...
  const result: { content: Array<{ type: 'text'; text: string }>; structuredContent?: Record<string, any> } =
    { content: [{ type: 'text' as const, text: lines.join('\n') }] };
  if (url) result.structuredContent = structuredFromInterp(data, url);
  return result;
}
...
const OWASP_CAT: Record<string, string> = {
...
const OWASP_FINDING: Record<string, (d?: string) => string> = {
...
function formatOwasp(data: any, url: string) {
  const a = data?.assessment ?? {};
  const checks: any[] = a.checks ?? [];
  const notObs: any[] = a.notObservable ?? [];
...
function formatImpersonation(data: any, url: string) {
  const a = data?.assessment ?? {};
  const checks: any[] = a.checks ?? [];
  const lines: string[] = [
  ...
  for (const c of checks) {
    ...
    if (c.findings?.length) {
      for (const f of c.findings) { const fn = IMP_FINDING[f.key]; lines.push(`  - ${fn ? fn(f.detail) : f.key}`); }
    } else {
...
function formatDomainChanges(data: any, url: string) {
  const lines: string[] = [`=== Domain Changes: ${data?.domain ?? ''} ===`];
  const changes: any[] = data?.changes ?? [];
  const tools: any[] = data?.watchedTools ?? [];
  const hLookups: any[] = data?.history?.lookups ?? [];
  const hChanges: any[] = data?.history?.changes ?? [];
  ...
  if (!data?.watched) {
    ...
    if (changes.length === 0) {
      ...
      for (const c of changes) lines.push(`  [${c.changedAt ?? ''}] ${c.endpoint ?? ''} ${c.kind}: ${c.summary}`);
    }
  }
  ...
  if (hLookups.length === 0) {
    ...
    if (paired.length === 0) {
      ...
      for (const c of hChanges) lines.push(`  ${c.endpoint ?? ''} ${c.kind}: ${c.summary}  (${String(c.since ?? '').slice(0, 10)} → ${String(c.changedAt ?? '').slice(0, 10)})`);
    }
  }
  lines.push('');
...
function formatWatchDomain(domain: string, results: Array<{ tool: string; endpoint: string; url?: string; error?: string }>, origin: string) {
  const okOnes = results.filter((r) => r.url);
  const failed = results.filter((r) => !r.url);
  const lines: string[] = [`=== Watch started: ${domain} ===`];
  if (okOnes.length) {
    ...
    lines.push('');
    for (const r of okOnes) lines.push(`  ${r.tool}: ${origin}${r.url}`);
  }
  if (failed.length) {
    lines.push('');
  ...
  return {
    ...
    ...(okOnes.length === 0 ? { isError: true as const } : {}),
  };
}

// SYNC with GOLIVE_* maps in the monorepo remote registry.
const GOLIVE_CK: Record<string, string> = {
...
const GOLIVE_FINDING: Record<string, (d?: string) => string> = {
...
const GOLIVE_VERDICT: Record<string, string> = { ready: 'READY', caution: 'CAUTION', not_ready: 'NOT READY' };

function formatGolive(data: any, url: string) {
  const a = data?.assessment ?? {};
  const checks: any[] = a.checks ?? [];
  const lines: string[] = [
  ...
  for (const c of checks) {
    ...
    if (c.findings?.length) {
      for (const f of c.findings) { const fn = GOLIVE_FINDING[f.key]; lines.push(`  - ${fn ? fn(f.detail) : f.key}`); }
    }
  }
  lines.push('');
  lines.push(`Full interactive report (share this link with the user): ${url}`);
...
export const tools: ToolDef[] = [
  ...
  },
  {
    ...
    handler: async ({ domain }) => {
      try { return formatResult(await callApi(`/api/util/subdomains?query=${enc(domain)}`), reportUrl('subdomains', 'query', domain)); }
      catch (e: any) { return errorResult(e.message); }
    },
  },
  {
    ...
    description:
      'Generate the typosquat/lookalike variants of a domain that phishers actually register — homoglyph swaps (l→1, o→0, rn→m), TLD swaps (.com→.co), character omissions, transpositions, repetitions, hyphenations — and check which of them are currently registered (live NS delegation via DoH). ' +
    ...
    handler: async ({ domain }) => {
      try { return formatResult(await callApi(`/api/util/lookalike?query=${enc(domain)}`), reportUrl('lookalike', 'query', domain)); }
      catch (e: any) { return errorResult(e.message); }
    },
  },
  {
  ...
  },
  {
    ...
    handler: async ({ cidr }) => {
      ...
      try {
        ...
        return { content: [{ type: 'text' as const, text }], structuredContent: { ...result, reportUrl: url } };
      } catch (e: any) { return errorResult(e.message); }
    },
  },
  {
  ...
  },
  {
    ...
    handler: async ({ host }) => {
      try { return formatOwasp(await callApi(`/api/util/owasp?host=${enc(host)}`), reportUrl('owasp', 'host', host)); }
      catch (e: any) { return errorResult(e.message); }
    },
  },
  {
    ...
    handler: async ({ domain }) => {
      try { return formatImpersonation(await callApi(`/api/util/impersonation?query=${enc(domain)}`), reportUrl('impersonation', 'query', domain)); }
      catch (e: any) { return errorResult(e.message); }
    },
  },
  {
    ...
    description:
      'Report what has changed for a domain over time — the security regressions and drift that DechoNet\'s daily monitoring has recorded across every watch on the domain (SSL grade, headers, DNS, OWASP posture, impersonation exposure, etc.). ' +
  ...
  },
  {
    ...
    description:
      'Start (or reuse) a daily DechoNet watch on a domain so that changes are recorded over time — SSL grade/issuer/expiry, DNS records, HTTP security headers, domain registration, and optionally OWASP posture and impersonation exposure. ' +
    ...
    handler: async ({ domain, tools }) => {
      ...
      if (!clean || chosen.length === 0) return errorResult('Provide a domain and at least one known tool.');
      const results: Array<{ tool: string; endpoint: string; url?: string; error?: string }> = [];
      for (const tool of chosen) {
        ...
        } catch (e: any) { results.push({ tool, endpoint, error: e.message }); }
      }
  ...
  },
  {
    ...
    description:
      'Check whether a domain is ready to launch or migrate — a go/no-go verdict over five essentials: DNS resolves to an IP, has propagated consistently across global resolvers, SSL/TLS is ready, the site is reachable over HTTPS, and the domain registration is not about to expire. ' +
